...
const enhancedLiveStreamSchema = z.object({
...
type EnhancedLiveStreamFormData = z.infer<typeof enhancedLiveStreamSchema>;
...
const qualities = [
  { value: "hd", label: "HD (720p)", icon: "📺" },
  { value: "fhd", label: "Full HD (1080p)", icon: "🎬" },
...
function generateEmbedUrl(platform: string, url: string): string {
  try {
    switch (platform) {
      case "twitch": {
        const channelMatch = url.match(/twitch\.tv\/([a-zA-Z0-9_]+)/);
        const channel = channelMatch?.[1];
        return channel ? `https://player.twitch.tv/?channel=${channel}&parent=localhost&parent=${window.location.hostname}` : url;
      }
      case "youtube": {
...
function CreateStreamDialog() {
  ...
  const { data: tournaments = [] } = useQuery<Tournament[]>({ queryKey: ["/api/tournaments"] });
  const { data: players = [] } = useQuery<Player[]>({ queryKey: ["/api/players"] });

  const form = useForm<EnhancedLiveStreamFormData>({
    resolver: zodResolver(enhancedLiveStreamSchema),
    defaultValues: {
      platform: "twitch",
      url: "",
      title: "",
  ...
  const onSubmit = (data: EnhancedLiveStreamFormData) => {
    const streamData: InsertLiveStream = {
      ...data,
      isLive: true,
      viewerCount: Math.floor(Math.random() * 50) + 10,
      embedUrl: generateEmbedUrl(data.platform, data.url),
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-card border-emerald-500/20 max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger data-testid="select-stream-platform" className="bg-muted border-emerald-500/20">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input 
                        {...field} 
                        placeholder="Friday Night Tournament Action"
                        className="bg-muted border-emerald-500/20"
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input 
                        {...field} 
                        placeholder="Downtown Billiards"
                        className="bg-muted border-emerald-500/20"
                        data-testid="input-pool-hall-name"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger data-testid="select-state" className="bg-muted border-emerald-500/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-gray-300">Stream Category</FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger data-testid="select-category" className="bg-muted border-emerald-500/20">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-gray-300">Video Quality</FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger data-testid="select-quality" className="bg-muted border-emerald-500/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-gray-300">Associated Match</FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger data-testid="select-match" className="bg-muted border-emerald-500/20">
                          <SelectValue placeholder="Select match (optional)" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        ...
                        {activeMatches.map((match) => (
                          <SelectItem key={match.id} value={match.id}>
                            <span className="text-emerald-400">LIVE:</span> {match.challenger} vs {match.opponent}
                          </SelectItem>
                        ))}
                        {scheduledMatches.map((match) => (
                          <SelectItem key={match.id} value={match.id}>
                            <span className="text-yellow-400">SCHED:</span> {match.challenger} vs {match.opponent}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-gray-300">Associated Tournament</FormLabel>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger data-testid="select-tournament" className="bg-muted border-emerald-500/20">
                          <SelectValue placeholder="Select tournament (optional)" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        <SelectItem value="">No specific tournament</SelectItem>
                        {activeTournaments.map((tournament) => (
                          <SelectItem key={tournament.id} value={tournament.id}>
                            🏆 {tournament.name} (${tournament.buyIn/100})
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              >
                {createStreamMutation.isPending ? (
                  <>
                    ...
                    <span className="ml-2">Creating Stream...</span>
                  </>
                ) : (
                  <>
                    <Video className="w-4 h-4 mr-2" />
...
function StreamFilters({ 
  ...
  const uniqueCities = useMemo(() => {
    return [...new Set(streams.map(s => s.city).filter(Boolean))].sort();
  }, [streams]);

  const uniqueStates = useMemo(() => {
    return [...new Set(streams.map(s => s.state).filter(Boolean))].sort();
  }, [streams]);

  const uniqueHalls = useMemo(() => {
    return [...new Set(streams.map(s => s.poolHallName).filter(Boolean))].sort();
  }, [streams]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="pl-10 bg-muted border-emerald-500/20"
          ...
          <div className="space-y-2">
            ...
            <Select value={filters.city} onValueChange={(value) => onFiltersChange({ ...filters, city: value })}>
              <SelectTrigger className="bg-muted border-emerald-500/20" data-testid="select-filter-city">
          ...
          <div className="space-y-2">
            ...
            <Select value={filters.state} onValueChange={(value) => onFiltersChange({ ...filters, state: value })}>
              <SelectTrigger className="bg-muted border-emerald-500/20" data-testid="select-filter-state">
          ...
          <div className="space-y-2">
            ...
            <Select value={filters.platform} onValueChange={(value) => onFiltersChange({ ...filters, platform: value })}>
              <SelectTrigger className="bg-muted border-emerald-500/20" data-testid="select-filter-platform">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={filters.category} onValueChange={(value) => onFiltersChange({ ...filters, category: value })}>
              <SelectTrigger className="bg-muted border-emerald-500/20" data-testid="select-filter-category">
          ...
          <div className="space-y-2">
            ...
            <Select value={filters.status} onValueChange={(value) => onFiltersChange({ ...filters, status: value })}>
              <SelectTrigger className="bg-muted border-emerald-500/20" data-testid="select-filter-status">
          ...
          <div className="space-y-2">
            ...
            <Select value={filters.sortBy} onValueChange={(value) => onFiltersChange({ ...filters, sortBy: value })}>
              <SelectTrigger className="bg-muted border-emerald-500/20" data-testid="select-sort-by">
...
function StreamCard({ stream }: { stream: LiveStream }) {
  ...
  const quality = qualities.find(q => q.value === stream.quality);

  const toggleStreamMutation = useMutation({
    mutationFn: () => apiRequest("PUT", `/api/live-streams/${stream.id}`, { 
      isLive: !stream.isLive,
      lastLiveAt: !stream.isLive ? new Date().toISOString() : stream.lastLiveAt
    }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/live-streams"] });
      toast({
        title: stream.isLive ? "Stream Ended" : "Stream Started",
        description: `${stream.title} is now ${stream.isLive ? "offline" : "live"}!`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const deleteStreamMutation = useMutation({
    mutationFn: () => apiRequest("DELETE", `/api/live-streams/${stream.id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/live-streams"] });
      toast({
        title: "Stream Deleted",
        description: `${stream.title} has been removed from the network.`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const updateViewersMutation = useMutation({
    mutationFn: () => {
      const newViewerCount = Math.floor(Math.random() * 200) + 20;
      const maxViewers = Math.max(stream.maxViewers || 0, newViewerCount);
      return apiRequest("PUT", `/api/live-streams/${stream.id}`, { 
  ...
  return (
    <Card className={`group hover:shadow-2xl transition-all duration-300 ${
      stream.isLive 
        ? 'border-red-500/50 bg-gradient-to-br from-red-900/10 via-transparent to-transparent shadow-red-500/10' 
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-400">
              ...
              <span className="font-semibold text-emerald-400">{stream.poolHallName}</span>
              <span>•</span>
              <span>{stream.city}, {stream.state}</span>
            </div>
          </div>
      ...
      <CardContent className="space-y-4">
        {/* Stream Preview */}
        <div className="relative group aspect-video bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg overflow-hidden">
          {stream.thumbnailUrl ? (
            <img 
              ...
              <div className="flex items-center justify-between text-white text-sm">
                <div className="flex items-center gap-2">
                  ...
                  {quality && (
                    <>
                      <span>•</span>
                      <span className="text-yellow-400">{quality.label}</span>
                    </>
                  )}
                </div>
          ...
          <button 
            onClick={handleWatch}
            className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 hover:scale-105"
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          ...
          <div>
            <div className="text-lg font-bold text-yellow-400">
              {stream.maxViewers?.toLocaleString() || 0}
            </div>
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">{quality?.label}</div>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <Button
            onClick={handleWatch}
            className={`flex-1 ${platform?.bgColor} ${platform?.color} border border-current hover:bg-opacity-20`}
            variant="outline"
          ...
          {stream.isLive && (
            <Button
              onClick={() => updateViewersMutation.mutate()}
              disabled={updateViewersMutation.isPending}
              size="sm"
              variant="outline"
              className="border-blue-500/30 text-blue-400 hover:bg-blue-500/10"
              data-testid={`button-refresh-viewers-${stream.id}`}
            >
              <RefreshCw className={`w-4 h-4 ${updateViewersMutation.isPending ? 'animate-spin' : ''}`} />
            </Button>
          )}
          ...
          <Button
            onClick={() => deleteStreamMutation.mutate()}
...
function StreamStats({ streams }: { streams: LiveStream[] }) {
  ...
  const peakViewers = streams.reduce((max, stream) => Math.max(max, stream.maxViewers || 0), 0);
  
  const platformStats = streams.reduce((acc, stream) => {
  ...
  const stateStats = streams.reduce((acc, stream) => {
    if (stream.state) {
  ...
  const topState = Object.entries(stateStats).sort(([,a], [,b]) => b - a)[0];

  return (
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
      <Card className="bg-gradient-to-br from-red-900/20 to-red-800/10 border-red-500/20">
        <CardContent className="p-4 text-center">
          <div className="text-2xl font-bold text-red-400 mb-1">{liveStreams.length}</div>
          <div className="text-xs text-gray-400">Live Now</div>
          <div className="w-2 h-2 bg-red-500 rounded-full mx-auto mt-2 animate-pulse" />
        </CardContent>
      </Card>

      <Card className="bg-gradient-to-br from-emerald-900/20 to-emerald-800/10 border-emerald-500/20">
        <CardContent className="p-4 text-center">
          <div className="text-2xl font-bold text-emerald-400 mb-1">{totalViewers.toLocaleString()}</div>
          <div className="text-xs text-gray-400">Total Viewers</div>
          <Eye className="w-4 h-4 mx-auto mt-2 text-emerald-400" />
        </CardContent>
      </Card>

      <Card className="bg-gradient-to-br from-yellow-900/20 to-yellow-800/10 border-yellow-500/20">
        <CardContent className="p-4 text-center">
          <div className="text-2xl font-bold text-yellow-400 mb-1">{peakViewers.toLocaleString()}</div>
          <div className="text-xs text-gray-400">Peak Viewers</div>
          <TrendingUp className="w-4 h-4 mx-auto mt-2 text-yellow-400" />
        </CardContent>
      </Card>

      <Card className="bg-gradient-to-br from-blue-900/20 to-blue-800/10 border-blue-500/20">
        <CardContent className="p-4 text-center">
          <div className="text-2xl font-bold text-blue-400 mb-1">{streams.length}</div>
          <div className="text-xs text-gray-400">Total Streams</div>
          <Video className="w-4 h-4 mx-auto mt-2 text-blue-400" />
        </CardContent>
      </Card>
      ...
      <Card className="bg-gradient-to-br from-purple-900/20 to-purple-800/10 border-purple-500/20">
        <CardContent className="p-4 text-center">
          <div className="text-lg font-bold text-purple-400 mb-1">
            {topPlatform ? platforms.find(p => p.value === topPlatform[0])?.icon : "🌐"}
          </div>
          <div className="text-xs text-gray-400">
            {topPlatform ? platforms.find(p => p.value === topPlatform[0])?.label : "No Data"}
          </div>
          <Globe className="w-4 h-4 mx-auto mt-2 text-purple-400" />
        </CardContent>
      </Card>
      ...
      <Card className="bg-gradient-to-br from-indigo-900/20 to-indigo-800/10 border-indigo-500/20">
        <CardContent className="p-4 text-center">
          ...
          <div className="text-xs text-gray-400">Top State</div>
          <MapPin className="w-4 h-4 mx-auto mt-2 text-indigo-400" />
        </CardContent>
      </Card>
    </div>
  );
  ...
  const filteredStreams = useMemo(() => {
    let filtered = streams.filter(stream => {
      const matchesSearch = !filters.search || 
        ...
        stream.city?.toLowerCase().includes(filters.search.toLowerCase());
      
      const matchesCity = !filters.city || stream.city === filters.city;
      const matchesState = !filters.state || stream.state === filters.state;
      ...
      return matchesSearch && matchesCity && matchesState && matchesPlatform && matchesCategory && matchesStatus;
    });
        ...
        <Card className="bg-card/30 border-emerald-500/20">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">
              {streams.length === 0 ? "No Streams Yet" : "No Streams Found"}
            </h3>
            <p className="text-gray-400 mb-6">
              {streams.length === 0 
                ? "Be the first to start streaming on the ActionLadder network!"
                : "Try adjusting your filters to find more streams."
              }
            </p>
            {streams.length === 0 ? (
              <CreateStreamDialog />
