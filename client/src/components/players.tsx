            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} placeholder="03-15" pattern="[0-9]{2}-[0-9]{2}" data-testid="input-player-birthday" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
...
function PlayerCard({ player }: { player: Player }) {
  ...
  const handleRemovePlayer = () => {
    if (confirm(`Are you sure you want to remove ${player.name}? Their match history will remain.`)) {
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center">
            ...
            <Badge 
              ...
              className={player.member ? "bg-neon-green/20 text-neon-green" : "bg-gray-600/20 text-gray-400"}
            >
          ...
          {/* Birthday */}
          {player.birthday && (
            <div className={`bg-gradient-to-r ${isBirthdayToday() ? 'from-yellow-600/20 to-transparent border-yellow-500/30' : 'from-blue-600/20 to-transparent border-blue-500/30'} border rounded-lg p-2`}>
              <div className="text-xs text-gray-400">Birthday:</div>
              <div className={`text-sm ${isBirthdayToday() ? 'text-yellow-400' : 'text-blue-400'}`}>
...
function BirthdayPlayers({ players }: { players: Player[] }) {
  ...
  const thisMonthBirthdays = players.filter(p => 
    p.birthday && p.birthday.startsWith(currentMonth) && p.birthday !== todayString
  );
    ...
    <Card className="bg-gradient-to-r from-yellow-600/20 to-transparent border border-yellow-500/30">
      ...
      <CardContent>
        {birthdayPlayers.length > 0 && (
          <div className="mb-4">
            <div className="text-sm font-semibold text-yellow-400 mb-2">🎉 Today's Birthdays:</div>
            {birthdayPlayers.map(player => (
              <div key={player.id} className="text-sm text-white flex items-center space-x-2">
                <span>•</span>
                <span>{player.name}</span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-black/60 backdrop-blur-sm border border-neon-green/20 shadow-felt">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-yellow-400">{hiDivisionPlayers.length}</div>
            <div className="text-sm text-gray-400">600+ Division</div>
          </CardContent>
        </Card>
        <Card className="bg-black/60 backdrop-blur-sm border border-neon-green/20 shadow-felt">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-accent">{loDivisionPlayers.length}</div>
