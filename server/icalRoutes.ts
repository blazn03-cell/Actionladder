...
import { requireAnyAuth, requireStaffOrOwner } from './auth';
...
export function createICalRoutes(storage: IStorage): Router {
   ...
   */
  const verifyPlayerAccess = async (req: any, playerId: string): Promise<boolean> => {
    try {
   ...
   */
  const sanitizeTokenResponse = (token: any) => ({
    id: token.id,
    name: token.name,
  ...
  router.get('/feed-url/:playerId', [feedRateLimit, requireAnyAuth], async (req: Request, res: Response) => {
    try {
      const { playerId } = req.params;
      const { hall, completed, start, end, tokenName } = req.query;
      
      // SECURITY CHECK: Verify user can access this player's data
      const hasAccess = await verifyPlayerAccess(req, playerId);
      if (!hasAccess) {
  ...
  // SECURITY: Requires authentication and authorization
  router.get('/tokens/:playerId', [tokenRateLimit, requireAnyAuth], async (req: Request, res: Response) => {
    try {
      const { playerId } = req.params;
  ...
  // SECURITY: Requires authentication and authorization
  router.post('/tokens/revoke', [tokenRateLimit, requireAnyAuth], async (req: Request, res: Response) => {
    try {
      ...
      const hasAccess = await verifyPlayerAccess(req, tokenData.playerId);
      if (!hasAccess) {
        return res.status(403).json({ 
  ...
  router.post('/cleanup-expired', [tokenRateLimit, requireStaffOrOwner], async (req: Request, res: Response) => {
    try {
