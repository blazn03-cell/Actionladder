...
// === PAYMENT ONBOARDING ROUTES ===
// SetupIntent collection for default payment methods at account creation
...
export function setupPaymentOnboardingRoutes(app: Express, storage: IStorage) {
  ...
  app.post("/api/payments/setup-intent", requireAnyAuth, async (req, res) => {
    try {
  ...
  app.post("/api/payments/attach", requireAnyAuth, async (req, res) => {
    try {
      // Get userId from authenticated session, never from request body
      const user = req.user as any;
      const userId = user.claims?.sub || user.id;
  ...
  app.get("/api/payments/methods", requireAnyAuth, async (req, res) => {
    try {
  ...
  app.post("/api/payments/set-default", requireAnyAuth, async (req, res) => {
    try {
      // Get userId from authenticated session, never from request body
      const user = req.user as any;
      const userId = user.claims?.sub || user.id;
  ...
  app.post("/api/payments/deactivate", requireAnyAuth, async (req, res) => {
    try {
      // Get userId from authenticated session, never from request body
      const user = req.user as any;
      const userId = user.claims?.sub || user.id;
  ...
  app.post("/api/webhooks/payment-onboarding", async (req, res) => {
