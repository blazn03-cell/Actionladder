...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/pricing/tiers", async (req, res) => {
    try {
      const tiers = Object.entries(MEMBERSHIP_TIERS).map(([key, config]) => ({
        ...
        name: config.name,
        price: config.price,
        monthlyPrice: config.price / 100, // Convert to dollars
        commissionRate: config.commissionRate / 100, // Convert to percentage
        perks: config.perks,
  ...
  // Calculate commission for challenge amount
  app.post("/api/pricing/calculate-commission", async (req, res) => {
    try {
